...
| ------- | -------------------------------- | -------------------------------- | -------------------- | ------------------------------------------- |
| 5.21.0  | [Download](https://downloads.getfiddler.com/win/Fiddler%20Everywhere%205.21.0.exe) | 5.17.0 +                         | -                    | Compatible with "no libfiddler.dll versions |


## Pre-requirements
  - Fiddler everywhere 5.21.0 (or another compatible version)
  ...
  - `fiddler.dll` & `hostpolicy.dll` from [Yui-Patch](https://github.com/project-Yui/Yui-patch). Download it from the [releases](https://github.com/project-Yui/Yui-patch/releases/).
    > Note that only `Yui-Patch` release `1.1.0 + ` are supported by Fiddler Everywhere 5.21.0. You can download `1.1.3` from release [1.1.3](https://github.com/project-Yui/Yui-patch/releases/tag/1.1.3) (latest)

    > Update: Important, that the current (as of 18/10/2024) the `latest release` - `1.1.3` is *supported by Fiddler Everyehere `5.21.0`+*
...
## Guide
  ...
  7. Now, you're in `root/resources/app/out/WebServer/`. Open `ClientApp/dist/` folder. Now you're in `root/resources/app/out/WebServer/ClientApp/dist`. Search a file named like `main-FHRSEMZ2.js`.
     > For `5.21.0` the file name is `main-FHRSEMZ2.js` but for other versions the text following `main-` is different.

  8. Copy the `main-FHRSEMZ2.js` likely named js to the same directory but rename it to `main-FHRSEMZ2.original.js`. So you need to append `.original` to end of the file name. So you have to files like this.
     ![Screenshot](https://github.com/user-attachments/assets/cbd5ce84-ae02-4cca-aa7e-48d54325f690)

  9. Open `main-FHRSEMZ2.js` like js (Not the one that have `.original` suffix. 
  ...
  12. Now, back into `root/resources/app/out/WebServer/` folder and create a `json` file named. `patch.json`
        ...
        ``` json
        {
            "ClientApp\\dist\\main-FHRSEMZ2.js": {
                "target": "ClientApp\\dist\\main-FHRSEMZ2.original.js",
                "content": "",
                "cur": 0,
                "start": 0,
                "end": 1
        ...
        ```
        > Note that the file names `main-FHRSEMZ2.js` & `main-FHRSEMZ2.original.js` is for version 5.21.0 as we discussed above. You need to replace it with tha names of the js files we've edited, and copied-renamed in `root/resources/app/out/WebServer/ClientApp/dist/`
